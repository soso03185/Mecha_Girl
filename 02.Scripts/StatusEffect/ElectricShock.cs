...
public class ElectricShock : ContinuousDamage
{
    float m_elapsedTime;
    public ElectricShock(Skill skill, ICreature target, float duration, float additionalDamage)
        : base(skill, "ElectricShock", target, duration, additionalDamage)
    {
    }

    protected override ContinuousDamage Clone(Skill skill, ICreature target, float duration, float additionalDamage)
    {
        return new ElectricShock(skill, target, duration, additionalDamage);
    }

    public override void ApplyEffect(Skill skill, ICreature target, float duration, float additionalDamage)
    {
        if ((target as MonsterScript).Attribute == Attribute.Electricity)
        {
