...
Varyings VertexShaderWork(Attributes input)
{
    ...
    output.vertex_Rim = TransformObjectToHClip(input.positionOS.xyz);
    output.normal = TransformObjectToWorldNormal(input.normalOS);
    output.WorldSpaceViewDirection = normalize(_WorldSpaceCameraPos.xyz - TransformObjectToWorld(input.positionOS.xyz));
    
    //// Move to NormalVector (for hit shader)
    // float NormalVectorScaleValue = 0.1f;
    // float3 worldPos = TransformObjectToWorld(input.positionOS).xyz;
    // float3 expandedPos = worldPos + input.normalOS * NormalVectorScaleValue;
    // output.positionCS = TransformWorldToHClip(expandedPos);
...
#include "GenshinBurn_LightingEquation.hlsl"
...
half4 ShadeFinalColor(Varyings input) : SV_TARGET
{
    ...
    float dValue = 0.03f; // 감소 값
    float4 burnTex = tex2D(_BurnTexture, input.uv); // Burn 텍스처 샘플링
    float burnEffect = saturate(_BurnAmount); // Burn 효과 값 (0~1로 제한)
    burnEffect -= dValue; // Burn 효과 감소

    // step 함수로 픽셀 출력 여부 결정
    half visibility = step(burnEffect, burnTex.r);
    clip(visibility - 1); // visibility 값이 1보다 작으면 픽셀 클리핑

    // 추가적인 색상 효과 계산
    float colorEffect = burnEffect + dValue; // 감소 값 추가
    half colorVisible = step(colorEffect, burnTex.r); // 색상 효과 출력 여부 결정

    // 색상 출력 조건
    if (colorVisible <= 0)
        color += _BurnColor; // Burn 색상 추가
    
    // _TransitionTime은 흰색에서 원래 색으로의 전환 속도를 제어합니다.
    float transition = saturate(_HitAmount / _HitDuration); // 전환 진행도 (0 ~ 0.5f 범위)
    color = lerp(_HitColor, color, transition); // _HitColor에서 원래 색상으로 선형 보간
    
    /// Rim Light 계산 ///
    float3 light = _MainLightPosition.xyz; // 주 광원 위치
    float4 colorDefault = float4(1, 1, 1, 1); // 기본 색상 (RGBA 모두 1로 초기화)

    // 월드 카메라 벡터와 노멀의 내적 값으로 방향 계산
    // 같은 방향: 1 (밝음), 90도: 0 (어두움)
    half face = saturate(dot(input.WorldSpaceViewDirection, input.normal));
    half3 rim = 1 - pow(face, _RimPower); // Rim Light 효과 계산

    // Emissive 항 계산
    colorDefault.rgb += rim * _RimInten * _RimColor; // Rim Light 색상 적용
    color *= colorDefault; // 최종 색상에 적용
